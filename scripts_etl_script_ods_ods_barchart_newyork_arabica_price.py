...
def insert_ods_table(conn_ods,df_final,ods_table):
    ...
    try:
        ...
        sql_copy = """
        COPY %s(contract_id,prev_contract_id,prev_open,prev,mo,last,prev_last,change,high,low,volume,oi,spread,ma_200,ma_50,date_id) 
...
def process_ods_table(stg_table, ods_table,conn_stg,conn_ods):
    ...
    select 
    contract,
    coalesce(lag(contract, '-1'::integer) OVER (PARTITION BY mo ORDER BY timing)::text,'NaN') AS prev_contract,
    last,
    lag(last, '-1'::integer) OVER (PARTITION BY mo ORDER BY timing) AS prev_last,
    round(last::numeric - lag(last, 1) OVER (PARTITION BY timing ORDER BY mo DESC)::numeric, 2) AS spread,
    avg(cast(last as float)) OVER (PARTITION BY mo ORDER BY timing ROWS BETWEEN 200 PRECEDING AND CURRENT ROW) AS ma_200,
    avg(cast(last as float)) OVER (PARTITION BY mo ORDER BY timing ROWS BETWEEN 50 PRECEDING AND CURRENT ROW) AS ma_50,
    timing,
    mo,
    change,
    prev_open,
    ...
    df_final = df_final[['contract_id_x','contract_id_y','prev_open','prev','mo','last','prev_last','change','high','low','volume','oi','spread','ma_200','ma_50','date_id']]    
    df_final.columns = ['contract_id','prev_contract_id','prev_open','prev','mo','last','prev_last','change','high','low','volume','oi','spread','ma_200','ma_50','date_id']
...
if __name__ == '__main__':
    ...
    ods_table = 'ods_barchart_newyork_arabica_price'
    stg_table = 'stg_barchart_newyork_arabica_price'
    script_name = 'ods_barchart_newyork_arabica_price.py'
